...
import FlashSaleCheckoutService, { FlashSaleCheckoutRequirement, } from "@/services/flash-sale/flash-sale-checkout.service";
import type { ShippingProviderCode, } from "@/services/shipping/shipping.types";
import shippingService from "@/services/shipping/shipping.service";
import settingsService from "@/services/settings/settings.service";
import FlashSaleRepository from "@/repositories/flash-sale/flash-sale.repository";
import { awardOrderRewardPointsTx, getSkuOptionSnapshotFromSku, } from "@/services/reward-point/reward-point.service";
...
export default class OrderService {
    ...
    */
    ...
    /**
   * Detail order milik customer tertentu.
   *
   * Customer hanya boleh melihat order miliknya sendiri.
   */
    static async getOrderByIdForUser(id: string, userId: string) {
        if (!id) {
    ...
    */
    static async getLatestOrders(limit = 5) {
        return OrderRepository.findLatest(limit);
    }
    /**
      * Total order berdasarkan status.
    */
    static async getOrdersByStatus(status: OrderStatus) {
        return OrderRepository.getTotalByStatus(status);
    }
    /**
      * Dashboard summary.
    */
    static async getDashboardSummary(): Promise<OrderDashboardSummary> {
        const [totalOrders, pendingPayments, pendingOrders, completedOrders, deletedOrders,] = await Promise.all([
            OrderRepository.getTotalOrders(),
            OrderRepository.getPendingPayments(),
            OrderRepository.getTotalByStatus(OrderStatus.PENDING),
            OrderRepository.getTotalByStatus(OrderStatus.COMPLETED),
    ...
    */
    static async ensureOrderExists(id: string) {
        const order = await OrderRepository.findById(id);
        if (!order) {
            throw new Error("Order tidak ditemukan.");
        }
    ...
    */
    static validatePaymentStatus(paymentStatus: PaymentStatus) {
        if (!paymentStatus) {
   ...
   */
    static async createOrder(input: CreateOrderInput) {
        ...
        for (const item of input.items) {
            ...
            if (!skuId) {
                throw new Error("SKU produk wajib dipilih.");
            }
            if (!Number.isInteger(item.quantity) ||
                item.quantity <= 0) {
                throw new Error("Quantity produk harus berupa angka bulat lebih dari 0.");
            }
            ...
            if (existing) {
                 ...
                 */
                if (item.preferredFlashSaleItemId) {
        ...
        return prisma.$transaction(async (tx) => {
            ...
            if (!user.isActive) {
                throw new Error("Customer tidak aktif.");
            }
            /**
             * ========================================================
             * 2. VALIDATE ADDRESS
            ...
            if (!address) {
                throw new Error("Alamat pengiriman tidak ditemukan atau bukan milik customer.");
            }
            /**
             * ========================================================
             * 3. GET PRODUCTS
            ...
            if (products.length !==
                ...
                const missingProduct = normalizedItems.find((item) => !foundIds.has(item.productId));
                throw new Error(`Produk ${missingProduct?.productId ??
                    ""} tidak ditemukan.`);
            }
            ...
            if (skus.length !==
                ...
                const missingSku = normalizedItems.find((item) => !foundSkuIds.has(item.skuId));
                throw new Error(`SKU ${missingSku?.skuId ??
                    ""} tidak ditemukan atau sudah tidak aktif.`);
            }
            const skuMap = new Map(skus.map((sku) => [
                sku.id,
                sku,
            ]));
            /**
            ...
            for (const item of normalizedItems) {
                ...
                if (sku.productId !==
                    product.id) {
                    throw new Error(`SKU "${sku.sku}" tidak sesuai dengan produk "${product.name}".`);
                }
                if (!sku.isActive) {
             ...
             * SKU-B stock -= 3
             */
            const stockRequirement = new Map<string, number>();
            for (const item of normalizedItems) {
                stockRequirement.set(item.skuId, (stockRequirement.get(item.skuId) ?? 0) +
                    item.quantity);
            }
            /**
             * ========================================================
            ...
            for (const item of normalizedItems) {
                ...
                const requiredQuantity = stockRequirement.get(sku.id) ?? 0;
                /**
                ...
                console.log("[ORDER-PRICING-RESULT]", {
                    orderUserId: input.userId,
                    productId: product.id,
                    skuId: sku.id,
                    finalPrice: pricing.finalPrice.toString(),
             ...
             * ========================================================
             */
            let voucherResult: Awaited<ReturnType<typeof VoucherService.validateAndCalculate>> | null = null;
            if (voucherCode) {
                voucherResult =
             ...
             * ========================================================
             */
            const voucherDiscount = voucherResult?.discountAmount ??
                new Prisma.Decimal(0);
            const discountedSubtotal = voucherResult?.finalSubtotal ??
                subtotal;
            /**
             * ========================================================
     ...
     */
             ...
             */
            console.log("[CREATE-ORDER-FLASH-SALE-REQUIREMENTS]", {
                orderId: order.id,
                userId: input.userId,
                requirements: flashSaleRequirements.map((requirement) => ({
                    flashSaleItemId: requirement.flashSaleItemId,
                    quantity: requirement.quantity,
                    price: requirement.price.toString(),
                })),
            });
            if (flashSaleRequirements.length >
                0) {
                await FlashSaleCheckoutService.consume({
                    userId: input.userId,
                    orderId: order.id,
                    requirements: flashSaleRequirements,
                }, tx);
            }
            /**
             * ========================================================
            ...
            if (voucherResult) {
                ...
                if (voucher.perUserLimit !==
                    null) {
                    await VoucherRepository.acquireUserVoucherLock(voucher.id, input.userId, tx);
                    const userUsageCount = await VoucherRepository.countUserUsage(voucher.id, input.userId, tx);
                    if (userUsageCount >=
                ...
                if (usageResult.count !==
                    1) {
                    throw new Error("Voucher sudah mencapai batas penggunaan. Silakan gunakan voucher lain.");
                }
                /**
                 * ------------------------------------------------------
            ...
            for (const [skuId, quantity,] of stockRequirement) {
                ...
                if (result.count !==
                    1) {
                    throw new Error(`Stok SKU "${sku.sku}" berubah sebelum transaksi selesai. Silakan coba lagi.`);
                }
                ...
                await tx.stockLedger.create({
                    data: {
                        ...
                        stockBefore,
                        stockAfter,
                        note: `Penjualan ${order.orderNumber} - SKU ${sku.sku}`,
                    },
                });
            }
            /**
    ...
    */
    static async updateOrder(id: string, input: UpdateOrderInput) {
        if (!id) {
            throw new Error("Order ID wajib diisi.");
        }
        if (!input.userId) {
        ...
        if (!Array.isArray(input.items) ||
            ...
            throw new Error("Minimal satu produk harus ada.");
        }
        /**
         * ============================================================
         * NORMALIZE ORDER ITEMS
  ...
  `;
       ...
       */
            if (order.userId !== input.userId) {
                throw new Error("Customer pada order tidak dapat diubah.");
            }
            /**
             * ==========================================================
            ...
            if (order.status !==
                OrderStatus.PENDING) {
                throw new Error("Order hanya dapat diedit saat berstatus PENDING.");
            }
            if (order.paymentStatus ===
                PaymentStatus.VERIFIED) {
                throw new Error("Order yang pembayarannya sudah terverifikasi tidak dapat diedit.");
            }
            /**
             * ==========================================================
             ...
             */
            const hasVoucher = order.voucherId !== null &&
                new Prisma.Decimal(order.voucherDiscount ?? 0).greaterThan(0);
            /**
             * ==========================================================
             * 3. VALIDATE CUSTOMER
             * ==========================================================
             ...
             */
            const legacyItem = order.items.find((item) => !item.skuId);
            if (legacyItem) {
                throw new Error("Order lama ini belum memiliki SKU canonical dan tidak dapat diedit. Silakan buat order baru.");
            }
            /**
             * ==========================================================
             * 6. GET PRODUCTS
             ...
             */
            const productIds = [
                ...new Set(finalItems.map((item) => item.productId)),
            ];
            const products = await tx.product.findMany({
                where: {
                    id: {
            ...
            if (products.length !==
                productIds.length) {
                const foundIds = new Set(products.map((product) => product.id));
                const missing = finalItems.find((item) => !foundIds.has(item.productId));
                throw new Error(`Produk ${missing?.productId ??
                    ""} tidak ditemukan.`);
            }
            const productMap = new Map(products.map((product) => [
                product.id,
             ...
             */
            const newSkuIds = [
                ...new Set(finalItems.map((item) => item.skuId)),
            ];
            const oldSkuIds = [
                ...new Set(order.items
                    .map((item) => item.skuId)
            ...
            if (skus.length !==
                skuIds.length) {
                const foundSkuIds = new Set(skus.map((sku) => sku.id));
                const missing = skuIds.find((skuId) => !foundSkuIds.has(skuId));
                throw new Error(`SKU ${missing ?? ""} tidak ditemukan.`);
            }
            const skuMap = new Map(skus.map((sku) => [
                sku.id,
                sku,
       ...
       */
            ...
            for (const item of order.items) {
                ...
                oldItemMap.set(key, (oldItemMap.get(key) ?? 0) +
                    item.quantity);
            }
            const newItemMap = new Map<string, number>();
            for (const item of finalItems) {
                const key = `${item.productId}::${item.skuId}`;
                newItemMap.set(key, (newItemMap.get(key) ?? 0) +
                    item.quantity);
            }
            let itemsChanged = oldItemMap.size !==
                newItemMap.size;
            if (!itemsChanged) {
                for (const [key, oldQuantity,] of oldItemMap) {
                    if (newItemMap.get(key) !== oldQuantity) {
                        itemsChanged = true;
                        break;
                    }
                }
       ...
       */
            ...
            for (const item of finalItems) {
                 ...
                 */
                const existingOrderItem = order.items.find((orderItem) => orderItem.productId ===
                    item.productId &&
                    orderItem.skuId ===
                        item.skuId);
                ...
                if (!itemsChanged &&
                    existingOrderItem) {
                    price =
                        new Prisma.Decimal(existingOrderItem.price);
                }
                else {
                    const pricing = await ProductPricingService.resolve(tx, {
                        productId: product.id,
       ...
       */
            const voucherDiscount = new Prisma.Decimal(order.voucherDiscount ??
                0);
            const subtotalAfterVoucher = Prisma.Decimal.max(subtotal.minus(voucherDiscount), new Prisma.Decimal(0));
            /**
             * ==========================================================
             * 14. SHIPPING + TOTAL
             * ==========================================================
             */
            const shipping = new Prisma.Decimal(shippingCost);
            const total = subtotalAfterVoucher.plus(shipping);
            /**
             * ==========================================================
            ...
            if (itemsChanged) {
                ...
                for (const skuId of affectedSkuIds) {
                    const oldQuantity = oldStockMap.get(skuId) ?? 0;
                    const newQuantity = newStockMap.get(skuId) ?? 0;
                 ...
                 */
                    if (delta > 0) {
                        ...
                        if (!currentSku) {
                            throw new Error(`SKU "${sku.id}" tidak ditemukan saat menambah quantity order.`);
                        }
                        /**
                         * --------------------------------------------------------
                         * SKU MUST BE ACTIVE
                        ...
                        if (!currentSku.isActive) {
                            throw new Error(`SKU "${currentSku.sku}" sedang tidak aktif dan tidak dapat menambah quantity order.`);
                        }
                        ...
                        if (stockBefore <
                            delta) {
                            throw new Error(`Stok SKU "${currentSku.sku}" tidak mencukupi. Stok tersedia: ${stockBefore}, tambahan yang dibutuhkan: ${delta}.`);
                        }
                        /**
                         * --------------------------------------------------------
                        ...
                        if (stockResult.count !==
                            1) {
                            throw new Error(`Stok SKU "${currentSku.sku}" berubah sebelum transaksi selesai. Silakan muat ulang halaman dan coba lagi.`);
                        }
                        /**
                         * --------------------------------------------------------
                        ...
                        await tx.stockLedger.create({
                            data: {
                                ...
                                stockBefore,
                                stockAfter,
                                note: `Penambahan quantity order ${order.orderNumber}: SKU ${currentSku.sku} bertambah ${delta}.`,
                            },
                        });
                        continue;
                    }
                    ...
                    if (!currentSku) {
                        throw new Error(`SKU "${sku.id}" tidak ditemukan saat mengembalikan stock.`);
                    }
                    const stockBefore = currentSku.stock;
                    /**
                     * --------------------------------------------------------
                    ...
                    if (stockResult.count !==
                        1) {
                        throw new Error(`Stock SKU "${currentSku.sku}" berubah sebelum stock dikembalikan. Silakan muat ulang halaman dan coba lagi.`);
                    }
                    const stockAfter = stockBefore +
                        restoreQuantity;
                    /**
                    ...
                    await tx.stockLedger.create({
                        data: {
                            ...
                            stockBefore,
                            stockAfter,
                            note: `Pengurangan quantity order ${order.orderNumber}: SKU ${currentSku.sku} berkurang ${restoreQuantity}.`,
                        },
                    });
                }
                /**
           ...
           */
                if (itemsChanged) {
                     ...
                     * terlebih dahulu.
                     */
                    await FlashSaleRepository.releasePurchasesByOrderId(tx, order.id);
                    /**
    ...
    static async updateStatus(id: string, status: OrderStatus) {
        ...
        if (status ===
            ...
            try {
                const completedOrder = await this.markAsCompleted(id);
                return {
                    success: true,
                    message: "Pesanan berhasil diselesaikan dan reward point telah diberikan.",
                    data: completedOrder,
                };
            }
            catch (error) {
                console.error("[ORDER_SERVICE_COMPLETE_ORDER_ERROR]", error);
                return {
                    success: false,
        ...
        try {
            ...
            if (order.status ===
                ...
                return {
                    success: false,
                    message: "Status pesanan yang sudah selesai atau dibatalkan tidak dapat diubah.",
                };
            }
            /**
             * ========================================================
            ...
            if (order.status ===
                ...
                return {
                    success: false,
                    message: "Pesanan sudah memiliki status tersebut.",
                };
            }
            /**
             * ========================================================
            ...
            if (order.status ===
                ...
                if (status ===
                    ...
                    return {
                        success: false,
                        message: "Pesanan tidak dapat diproses sebelum pembayaran diverifikasi.",
                    };
                }
                if (status !==
                    OrderStatus.PROCESSING &&
                    ...
                    return {
                        success: false,
                        message: "Pesanan dengan status PENDING hanya dapat diproses atau dibatalkan.",
                    };
                }
            }
            /**
            ...
            if (order.status ===
                ...
                return {
                    success: false,
                    message: "Pesanan yang menunggu pembayaran hanya dapat dilanjutkan ke verifikasi atau dibatalkan.",
                };
            }
            /**
             * --------------------------------------------------------
            ...
            if (order.status ===
                ...
                if (status !==
                    ...
                    return {
                        success: false,
                        message: "Pesanan yang sedang menunggu verifikasi hanya dapat diproses atau dibatalkan.",
                    };
                }
            }
            /**
            ...
            if (order.status ===
                ...
                return {
                    success: false,
                    message: "Pesanan yang sedang diproses hanya dapat ditandai sebagai dikirim atau dibatalkan.",
                };
            }
            /**
             * --------------------------------------------------------
            ...
            if (order.status ===
                ...
                return {
                    success: false,
                    message: "Pesanan yang sudah dikirim hanya dapat diselesaikan atau dibatalkan.",
                };
            }
            /**
             * ========================================================
            ...
            if (status ===
                OrderStatus.CANCELLED) {
                const cancelledOrder = await this.cancelOrder(id);
                return {
                    success: true,
             ...
             */
            const updatedOrder = await OrderRepository.updateStatus(id, status, order.status);
            /**
             * ========================================================
            ...
            if (!updatedOrder) {
                return {
                    success: false,
                    message: "Status pesanan telah berubah oleh proses lain. Silakan refresh data dan coba lagi.",
                };
            }
            /**
             * ========================================================
            ...
            console.error("[ORDER_SERVICE_UPDATE_STATUS_ERROR]", error);
            return {
                success: false,
                message: error instanceof Error
                    ? error.message
    ...
    */
    private static async cancelOrderInTransaction(tx: Prisma.TransactionClient, id: string, userId?: string) {
        /**
         * ========================================================
         * 1. LOCK ORDER ROW
         * ========================================================
   ...
   * ========================================================
   *
   * Customer hanya boleh membatalkan order
   * sebelum pembayaran diverifikasi.
   *
   * Internal cancellation tidak dibatasi
   * oleh policy customer ini.
   */
        if (userId &&
            currentOrder.status !== OrderStatus.PENDING &&
            currentOrder.status !== OrderStatus.WAITING_PAYMENT) {
            throw new Error("Pesanan tidak dapat dibatalkan pada tahap ini.");
        }
        /**
         * ========================================================
        ...
        if (currentOrder.deletedAt) {
            throw new Error("Order yang berada di Trash tidak dapat dibatalkan.");
        }
        ...
        if (currentOrder.paymentStatus ===
            PaymentStatus.VERIFIED) {
            throw new Error("Order dengan pembayaran VERIFIED tidak dapat dibatalkan.");
        }
        /**
         * ========================================================
         ...
         */
        const legacyItem = currentOrder.items.find((item) => !item.skuId);
        if (legacyItem) {
            throw new Error(`Order ${currentOrder.orderNumber} memiliki OrderItem lama tanpa skuId dan tidak dapat dibatalkan menggunakan stock engine SKU baru.`);
        }
        /**
         * ========================================================
        ...
        for (const item of currentOrder.items) {
            ...
            if (!Number.isInteger(item.quantity) ||
                item.quantity <= 0) {
                throw new Error(`Quantity OrderItem "${item.id}" tidak valid saat pembatalan order.`);
            }
            skuQuantities.set(item.skuId, (skuQuantities.get(item.skuId) ?? 0) +
                item.quantity);
        }
        /**
         * ========================================================
         ...
         * ========================================================
         */
        const skuIds = Array.from(skuQuantities.keys());
        const skus = await tx.productSku.findMany({
            where: {
                id: {
                    in: skuIds,
        ...
        if (skus.length !==
            ...
            throw new Error(`SKU "${missingSkuId ?? ""}" tidak ditemukan saat pembatalan order.`);
        }
        const skuMap = new Map(skus.map((sku) => [
            sku.id,
            sku,
   ...
   */
         ...
         */
        await VoucherLifecycleService.releaseForCancelledOrder(currentOrder.id, currentOrder.paymentStatus, tx);
        /**
         * ========================================================
    ...
    static async updatePaymentStatus(id: string, paymentStatus: PaymentStatus) {
        ...
        `;
            ...
            if (order.status ===
                OrderStatus.CANCELLED) {
                throw new Error("Pembayaran order yang sudah dibatalkan tidak dapat diubah.");
            }
            /**
             * ========================================================
            ...
            if (order.status ===
                OrderStatus.COMPLETED) {
                throw new Error("Pembayaran order yang sudah selesai tidak dapat diubah.");
            }
            /**
             * ========================================================
            ...
            if (!allowedNextStatuses.includes(paymentStatus)) {
                throw new Error(`Perubahan status pembayaran dari ${order.paymentStatus} ke ${paymentStatus} tidak diizinkan.`);
            }
            /**
             * ========================================================
    ...
    static async markAsPaid(id: string) {
        return this.updatePaymentStatus(id, PaymentStatus.VERIFIED);
    }
    /**
     * ============================================================
    ...
    static async markAsCompleted(id: string) {
        ...
        `;
            ...
            if (order.status ===
                OrderStatus.CANCELLED) {
                throw new Error("Order yang sudah dibatalkan tidak dapat diselesaikan.");
            }
            /**
             * ========================================================
            ...
            if (order.status !==
                OrderStatus.SHIPPING) {
                throw new Error("Order harus berstatus SHIPPING sebelum dapat diselesaikan.");
            }
            /**
             * ========================================================
            ...
            await awardOrderRewardPointsTx(tx, completedOrder);
    ...
    static async deleteOrder(id: string) {
        ...
        `;
             ...
             */
            if (order.deletedAt) {
                throw new Error("Order sudah berada di Trash.");
            }
            /**
             * ========================================================
             ...
             * ke Trash.
             */
            const isFinalStatus = order.status ===
                OrderStatus.COMPLETED ||
                order.status ===
                    OrderStatus.CANCELLED;
            if (!isFinalStatus) {
                throw new Error("Order yang masih aktif tidak dapat dipindahkan ke Trash. Selesaikan atau batalkan order terlebih dahulu.");
            }
            /**
             * ========================================================
             * 6. SOFT DELETE
    ...
    static async restoreOrder(id: string) {
        ...
        `;
             ...
             */
            if (!order.deletedAt) {
                throw new Error("Order tidak berada di Trash.");
            }
            /**
             * ========================================================
            ...
            const isFinalStatus = order.status ===
                ...
                order.status ===
                    OrderStatus.CANCELLED;
            if (!isFinalStatus) {
                throw new Error("Hanya order COMPLETED atau CANCELLED yang dapat dipulihkan.");
            }
            /**
             * ========================================================
             * 6. RESTORE ORDER
     ...
     */
    static async forceDeleteOrder(id: string) {
        /**
         * ==========================================================
         * VALIDATE ID
         * ==========================================================
        ...
        `;
             ...
             */
            if (!order.deletedAt) {
                throw new Error("Order harus dipindahkan ke Trash terlebih dahulu sebelum dihapus permanen.");
            }
            /**
             * ========================================================
             * 5. REQUIRE FINAL STATUS
            ...
            const isFinalStatus = order.status ===
                ...
                order.status ===
                    OrderStatus.CANCELLED;
            if (!isFinalStatus) {
                throw new Error("Hanya order COMPLETED atau CANCELLED yang dapat dihapus permanen.");
            }
            /**
             * ========================================================
             * 6. FORCE DELETE
    ...
    static async createCheckoutOrder(userId: string, addressId: string, paymentChannelId: string, notes?: string | null, shippingProvider: ShippingProviderCode = "INTERNAL", voucherCode?: string | null) {
        ...
        try {
            ...
            */
            ...
            const normalizedShippingProvider = shippingProvider
                .trim()
            ...
            */
            ...
            if (!address) {
                return {
                    success: false,
                    message: "Alamat pengiriman tidak ditemukan atau tidak valid.",
                };
            }
            /**
              * ========================================================
            ...
            */
            if (address.latitude === null ||
                ...
                return {
                    success: false,
                    message: "Lokasi alamat pengiriman belum memiliki koordinat.",
                };
            }
            /**
              * ========================================================
            ...
            */
            ...
            if (!paymentChannel) {
                return {
                    success: false,
                    message: "Metode pembayaran tidak tersedia atau sudah tidak aktif.",
                };
            }
            const paymentMethod = paymentChannel.type === "QRIS"
                ? PaymentMethod.QRIS
                : PaymentMethod.BANK_TRANSFER;
            if (!paymentChannel) {
                return {
            ...
            */
            ...
            /**
              ...
              * ========================================================
            */
            if (normalizedShippingProvider ===
                "INTERNAL") {
                shippingService.registerInternalProvider({
                    enabled: settings.internalShippingEnabled,
            ...
            /**
              ...
              * diregistrasikan.
            */
            if (!shippingService.hasProvider(normalizedShippingProvider)) {
                return {
                    success: false,
                    message: "Provider pengiriman tidak tersedia.",
                };
            }
            /**
              * ========================================================
            ...
            */
            const order = await prisma.$transaction(async (tx) => {
                /**
         * ====================================================
         * LOCK CART + READ CURRENT CART
         * ====================================================
         *
         * Cart menjadi concurrency gate untuk checkout.
         *
         * Checkout kedua akan menunggu sampai checkout pertama
         * selesai menggunakan Cart yang sama.
         *
         * Setelah lock diperoleh, Cart dibaca kembali dari
         * transaction yang sama sehingga state yang digunakan
         * selalu merupakan state terbaru.
         */
                const lockedCart = await tx.$queryRaw<Array<{
                    id: string;
                }>> `
          SELECT "id"
          FROM "Cart"
        ...
        `;
                 ...
                 * ====================================================
                 *
                 * Generate setelah Cart berhasil dikunci dan terbukti
                 * masih memiliki item.
                 */
                ...
                */
                for (const item of cart.items) {
                    ...
                    if (!item.sku.isActive) {
                        throw new Error(`SKU "${item.sku.sku}" sedang tidak aktif.`);
                    }
                    if (!Number.isInteger(item.quantity) ||
                        item.quantity <= 0) {
                ...
                */
                 ...
                 * ========================================================
                 */
                const subtotal = normalizedItems.reduce((total, item) => total.plus(item.subtotal), new Prisma.Decimal(0));
                /**
                  * ========================================================
                  * VOUCHER CALCULATION
                  * ========================================================
                ...
                */
                /**
                  ...
                  * Ongkir selalu dihitung ulang oleh server.
                */
                const shippingResult = await shippingService.getQuote({
                ...
                */
                if (!shippingResult.available) {
                    throw new Error(shippingResult.reason ??
...
data: {
    ...
    items: {
        include: {
            ...
            sku: true,
                 ...
                 */
                if (flashSaleRequirements.length > 0) {
                    await FlashSaleCheckoutService.consume({
                        userId,
                ...
                */
                if (voucherResult) {
                    ...
                    if (voucher.perUserLimit !== null) {
                        await VoucherRepository.acquireUserVoucherLock(voucher.id, userId, tx);
                        const userUsageCount = await VoucherRepository.countUserUsage(voucher.id, userId, tx);
                        if (userUsageCount >=
                            voucher.perUserLimit) {
                            throw new Error("Anda sudah mencapai batas penggunaan voucher ini.");
                        }
         ...
         */
                ...
                for (const item of normalizedItems) {
                    if (!item.skuId) {
                        throw new Error(`SKU untuk produk "${item.productName}" tidak valid.`);
                    }
                    if (!Number.isInteger(item.quantity) ||
                        item.quantity <= 0) {
                        throw new Error(`Quantity untuk SKU "${item.skuId}" tidak valid.`);
                    }
                    stockRequirements.set(item.skuId, (stockRequirements.get(item.skuId) ?? 0) +
                        item.quantity);
                }
                /**
                 * ====================================================
                 ...
                 * ====================================================
                 */
                const lockedSkuIds = Array.from(stockRequirements.keys()).sort();
                for (const skuId of lockedSkuIds) {
                    const lockedSku = await tx.$queryRaw<Array<{
                        id: string;
                    }>> `
              SELECT "id"
              FROM "ProductSku"
            ...
            `;
                    if (lockedSku.length ===
                        0) {
                        throw new Error(`SKU "${skuId}" tidak ditemukan saat checkout.`);
                    }
                }
                /**
                 * ====================================================
                ...
                for (const [skuId, quantity,] of lockedSkuIds.map((id) => [
                    ...
                    await tx.stockLedger.create({
                        data: {
                            ...
                            stockBefore,
                            stockAfter,
                            note: `Penjualan ${createdOrder.orderNumber} - SKU ${currentSku.sku}`,
                        },
                    });
                }
                /**
    ...
    /**
      ...
      * ============================================================
    */
    static async confirmQrisPayment(userId: string, orderId: string) {
        try {
            /**
              * ========================================================
              * VALIDATE INPUT
            ...
            */
            if (settings.latitude === null ||
                ...
                return {
                    success: false,
                    message: "Lokasi toko belum dikonfigurasi dengan benar.",
                };
            }
            /**
              * ========================================================
            ...
            */
            return {
                success: true,
                message: "Konfirmasi pembayaran berhasil dikirim. Pembayaran Anda sedang menunggu verifikasi admin.",
            };
        }
        catch (error) {
            console.error("[CONFIRM_QRIS_PAYMENT_ERROR]", error);
            return {
                success: false,
                message: error instanceof Error
                    ? error.message
