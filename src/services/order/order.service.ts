...
export default class OrderService {
  ...
  static async createOrder(input: CreateOrderInput) {
    ...
    if (
      ...
      input.paymentMethod !== PaymentMethod.QRIS
    ) {
    ...
    for (const item of input.items) {
      ...
      const customerNote = item.customerNote?.trim() || null;
      ...
      if (existing) {
        ...
        if (customerNote !== null) {
          existing.customerNote = customerNote;
        }
        /**
        ...
        if (item.preferredFlashSaleItemId) {
          existing.preferredFlashSaleItemId = item.preferredFlashSaleItemId;
        }
      } else {
        itemMap.set(itemKey, {
          productId,
          ...
          customerNote,
          preferredFlashSaleItemId: item.preferredFlashSaleItemId ?? null,
        });
      }
    }
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (products.length !== productIds.length) {
        ...
        throw new Error(
          `Produk ${missingProduct?.productId ?? ""} tidak ditemukan.`,
        );
      }
      ...
      if (skus.length !== skuIds.length) {
        ...
        const missingSku = normalizedItems.find(
          (item) => !foundSkuIds.has(item.skuId),
        );
        throw new Error(
          `SKU ${
            missingSku?.skuId ?? ""
      ...
      for (const item of normalizedItems) {
        stockRequirement.set(
          item.skuId,
          (stockRequirement.get(item.skuId) ?? 0) + item.quantity,
        );
      }
      /**
       * ========================================================
      ...
      for (const item of normalizedItems) {
         ...
         */
        if (pricing.isFlashSaleApplied && pricing.flashSaleItemId) {
        ...
        const weightGrams = skuSnapshot.weightSku
          ? null
       ...
       * ========================================================
       */
      let voucherResult: Awaited<
        ReturnType<typeof VoucherService.validateAndCalculate>
      > | null = null;
      if (voucherCode) {
        voucherResult = await VoucherService.validateAndCalculate(
          {
            code: voucherCode,
            userId: input.userId,
            subtotal,
      ...
      const voucherDiscount =
        voucherResult?.discountAmount ?? new Prisma.Decimal(0);
  ...
  `;
       ...
       * ==========================================================
       */
      const productIds = [...new Set(finalItems.map((item) => item.productId))];
      const products = await tx.product.findMany({
        where: {
          id: {
            in: productIds,
      ...
      if (products.length !== productIds.length) {
        const foundIds = new Set(products.map((product) => product.id));
        const missing = finalItems.find(
          (item) => !foundIds.has(item.productId),
        );
        throw new Error(`Produk ${missing?.productId ?? ""} tidak ditemukan.`);
      }
      const productMap = new Map(
        products.map((product) => [product.id, product]),
      );
       ...
       * ==========================================================
       */
      const newSkuIds = [...new Set(finalItems.map((item) => item.skuId))];
      const oldSkuIds = [
        ...new Set(
          order.items
            .map((item) => item.skuId)
      ...
      const skuIds = [...new Set([...newSkuIds, ...oldSkuIds])];
      const skus = await tx.productSku.findMany({
        where: {
          id: {
            in: skuIds,
      ...
      if (historicalSkuIds.length > 0) {
        const historicalLedgers = await tx.stockLedger.findMany({
          where: {
            orderId: order.id,
            skuId: {
              in: historicalSkuIds,
        ...
        for (const ledger of historicalLedgers) {
          ...
          const currentNet = oldStockMap.get(ledger.skuId) ?? 0;

          oldStockMap.set(ledger.skuId, currentNet + ledger.quantity);
        }
        ...
        for (const skuId of historicalSkuIds) {
          const netLedgerQuantity = oldStockMap.get(skuId) ?? 0;

          oldStockMap.set(skuId, Math.max(0, -netLedgerQuantity));
        }
      }
      /**
       * ==========================================================
      ...
      for (const item of finalItems) {
        ...
        newStockMap.set(
          item.skuId,
          (newStockMap.get(item.skuId) ?? 0) + physicalQuantity,
        );
      }
      /**
       * ==========================================================
      ...
      for (const item of order.items) {
        ...
        oldItemMap.set(key, (oldItemMap.get(key) ?? 0) + item.quantity);
      }
      const newItemMap = new Map<string, number>();
      for (const item of finalItems) {
        const key = `${item.productId}::${item.skuId}`;
        newItemMap.set(key, (newItemMap.get(key) ?? 0) + item.quantity);
      }
      let itemsChanged = oldItemMap.size !== newItemMap.size;
      if (!itemsChanged) {
      ...
      if (itemsChanged) {
        for (const [skuId, newQuantity] of newStockMap) {
           ...
           * Gunakan StockLedger.
           */
          const oldPhysicalQuantity = oldStockMap.get(skuId) ?? 0;
           ...
           * oleh order ini.
           */
          const availableStock = sku.stock + oldPhysicalQuantity;
      ...
      for (const item of finalItems) {
        ...
        const weightGrams = skuSnapshot.weightSku
          ? null
          : !itemsChanged && existingOrderItem?.weightGrams != null
       ...
       */
      const voucherDiscount = new Prisma.Decimal(order.voucherDiscount ?? 0);
      const subtotalAfterVoucher = Prisma.Decimal.max(
        subtotal.minus(voucherDiscount),
  ...
  static async updateStatus(id: string, status: OrderStatus) {
    ...
    if (status === OrderStatus.COMPLETED) {
      try {
        const { completedOrder, rewardResult } = await this.markAsCompleted(id);

        /**
         * CUSTOMER ORDER COMPLETED NOTIFICATION
         */
        try {
          ...
          console.error(
            "[CUSTOMER_ORDER_COMPLETED_NOTIFICATION_ERROR]",
            notificationError,
          );
        }

        /**
         * CUSTOMER REWARD POINTS NOTIFICATION
         */
        try {
          if (
            completedOrder.userId &&
            rewardResult.awarded &&
            rewardResult.points > 0
          ) {
            await notificationService.createCustomerRewardPointsNotification({
              userId: completedOrder.userId,
              orderId: completedOrder.id,
              orderNumber: completedOrder.orderNumber,
          ...
          console.error(
            "[CUSTOMER_REWARD_POINTS_NOTIFICATION_ERROR]",
            notificationError,
          );
        }
    ...
    try {
       ...
       * ========================================================
       */
      const isPaymentVerified = order.paymentStatus === PaymentStatus.VERIFIED;
      /**
       * ========================================================
       ...
       */
      if (order.status === OrderStatus.PENDING) {
        if (status === OrderStatus.PROCESSING && !isPaymentVerified) {
          return {
            success: false,
            message:
       ...
       * ==========================================================
       *
       * Hanya kirim notifikasi ketika order berubah
       * dari status sebelumnya menjadi SHIPPING.
       */
      if (
        order.status !== OrderStatus.SHIPPING &&
        status === OrderStatus.SHIPPING &&
        ...
        try {
          ...
          console.error(
            "[CUSTOMER_ORDER_STATUS_NOTIFICATION_ERROR]",
            notificationError,
          );
        }
      }
   ...
   */
  private static async cancelOrderInTransaction(
    ...
    if (currentOrder.paymentStatus === PaymentStatus.VERIFIED) {
      throw new Error(
    ...
    for (const item of currentOrder.items) {
      ...
      skuQuantities.set(
        item.skuId,
        (skuQuantities.get(item.skuId) ?? 0) + item.quantity,
      );
    }
    /**
     * ========================================================
    ...
    if (historicalSkuIds.length > 0) {
      ...
      for (const ledger of historicalLedgers) {
        ...
        const currentNet = historicalStockMap.get(ledger.skuId) ?? 0;

        historicalStockMap.set(ledger.skuId, currentNet + ledger.quantity);
      }
       ...
       */
      for (const skuId of historicalSkuIds) {
        const netQuantity = historicalStockMap.get(skuId) ?? 0;

        historicalStockMap.set(skuId, Math.max(0, -netQuantity));
      }
    }
    /**
     * ========================================================
    ...
    for (const [skuId, quantity] of skuQuantities) {
      ...
      if (!product) {
        throw new Error(
          `Produk untuk SKU "${sku.sku}" tidak ditemukan saat pembatalan order.`,
        );
      }
      /**
       * ========================================================
       ...
       * diambil oleh order berdasarkan StockLedger.
       */
      const physicalReservedQuantity = historicalStockMap.get(skuId) ?? 0;
       ...
       */
      if (physicalReservedQuantity <= 0) {
        continue;
      }

      const quantityToRestore = physicalReservedQuantity;

      /**
       * Validasi reservation historis.
       */
      if (!Number.isInteger(quantityToRestore) || quantityToRestore <= 0) {
  ...
  static async updatePaymentStatus(id: string, paymentStatus: PaymentStatus) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      const allowedTransitions: Record<PaymentStatus, PaymentStatus[]> = {
        ...
        [PaymentStatus.REJECTED]: [PaymentStatus.VERIFIED],
        [PaymentStatus.VERIFIED]: [],
      };
      /**
       * ========================================================
      ...
      if (
        ...
        nextOrderStatus = OrderStatus.WAITING_PAYMENT;
      }
      /**
       * ========================================================
  ...
  static async submitPaymentProof(
    ...
    try {
       ...
       * ========================================================
       */
      uploadedImagePath = await StorageService.save(input.file);
      /**
       * ========================================================
       * DATABASE TRANSACTION
       *
      ...
      if (
        ...
        paymentProof.oldImagePath !== uploadedImagePath
      ) {
        try {
  ...
  static async createCheckoutOrder(
    ...
    try {
      ...
      const normalizedSelectedItemIds =
        selectedItemIds?.map((id) => id.trim()).filter(Boolean) ?? null;
      if (
       ...
       * ========================================================
       */
      if (normalizedShippingProvider === "INTERNAL") {
        shippingService.registerInternalProvider({
          enabled: settings.internalShippingEnabled,
          name: settings.internalShippingName ?? "Kurir Internal",
      ...
      const order = await prisma.$transaction(async (tx) => {
        ...
        if (
          normalizedSelectedItemIds !== null &&
          selectedCartItems.length !== normalizedSelectedItemIds.length
        ) {
          throw new Error(
         ...
         * ====================================================
         */
        for (const [skuId, quantity] of lockedSkuIds.map(
          (id) => [id, stockRequirements.get(id) ?? 0] as const,
        )) {
          if (quantity <= 0) {
            continue;
          }
          ...
          if (!checkoutProduct) {
            throw new Error(
              `Produk untuk SKU "${currentSku.sku}" tidak ditemukan saat checkout.`,
            );
          }
          /**
           * --------------------------------------------------
  ...
  static async confirmQrisPayment(userId: string, orderId: string) {
    try {
       ...
       * ========================================================
       */
      if (order.paymentChannel?.type !== "QRIS") {
        return {
          success: false,
